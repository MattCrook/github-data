...
const objectKeysNumber = Object.keys(githubData);
console.log(objectKeysNumber);
console.log(objectKeysNumber.length);  // 30

console.log(githubData)

// using for loop
for (let i = 0; i < githubData.length; i++) {
    let data = githubData[i];
    let id = data.id;
    console.log(id);
    console.log(data.payload);
    if (id === "8030403992") {
        console.log("Returned the data", data.payload.commits[0].message);
    }
}

// using higher order function...filter() method
githubData.filter((data) => data.id === "8030403992");
const filteredData = githubData.filter((data) => data.id === "8030403992");
//console.log(filteredData);
console.log(filteredData[0].payload.commits[0].message);

console.log(filteredData[0].payload && filteredData[0].payload.commits[0] && filteredData[0].payload.commits[0].message); // adding guards


// to explain above more;
// because it is an array of objects, we must grab the object out of the array first. So:
// filteredData[0] === data // true
...
// "Added clarification for where to clone static site"
